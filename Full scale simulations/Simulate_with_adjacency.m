...
f_fixeddegree = figure('Renderer', 'painters', 'Position', [50 800 800 350]); box on; hold on;
% title('Fixed degree network', 'FontSize', titlefont)
...
for i = 1:3
    ...
    fullplot = plot(tfull, abs(zfull), '-', 'LineWidth', 4, 'Color', '#0072BD');
    ZOAplot  = plot(TOA, abs(ZOA), '-', 'LineWidth', 3, 'Color', fdpars.color);
...
f_random = figure('Renderer', 'painters', 'Position', [50 800 800 350]); box on; hold on;
xlim([tnow, tend]); ylim([0, 1]);
A = 0;
...
f_scalefree = figure('Renderer', 'painters', 'Position', [50 800 800 350]); box on; hold on;
xlim([tnow, tend]); ylim([0, 1]);
A = 0;
