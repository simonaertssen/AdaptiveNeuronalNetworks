function [tout,xout] = DOPRI_threshold(originalfunc,ta,tb,x0,h,p)
    ...
    for i = 1:(npts-1)
        ...
        K1 = h*func(ta, x0);
        K2 = h*func(tout(i), xout(:,i) + K1*0.5);
        K3 = h*func(tout(i), xout(:,i) + K2*0.5);
        K4 = h*func(tout(i), xout(:,i) + K3);
        tmp = xout(:,i) + 1/6*(K1 + 2*K2 + 2*K3 + K4);
...
%         indicesmax = tmp > pi;
%         indicesmin = tmp < -pi;
%         orarray = bitand(indicesmax, indicesmin);
%         if (sum(orarray) > 0)
...
%         end
%         
%         tmp(indicesmax) = tmp(indicesmax) - 2*pi;
%         tmp(indicesmin) = tmp(indicesmin) + 2*pi;
% 
%         xout(:,i+1) = tmp;
