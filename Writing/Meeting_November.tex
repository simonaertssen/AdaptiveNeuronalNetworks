...
\Delta W(s)_S =
\begin{cases}
\: \: \: A_{p} \cdot \exp \left(\frac{s}{\tau_p}\right) & \text{for } s \leq 0 \\
-A_{n} \cdot \exp \left(\frac{-s}{\tau_n}\right)  & \text{for } s > 0
\end{cases} \label{eq:learningwindowKempter1999}
\end{align}
where $\tau_p = \tau_n = 20$ ms.


\subsection{Waddington 2014}
\begin{align}
\Delta W(s)_W =  \eta \left[1-\frac{\left(s-\alpha\right)^{2}}{\alpha^{2}}\right] \cdot \exp \left(\frac{-\left|s - \alpha\right|}{\alpha}\right) \label{eq:learningwindowWaddington2014}
\end{align}
We will use $\eta = 0.1$ and $\alpha = 4.0$ ms and $A_p = 0.005$ and $A_n = 0.00525$. This brings the integral 


\subsection{Chrol-Cannon 2012}
\begin{align}
\Delta W(s)_W = A_{p} \cdot \exp \left(\frac{-\left(s - 15 \right)^{2}}{200}\right) - A_{n} \cdot \exp \left(\frac{-\left(s - 15\right)^{2}}{2000}\right)  \label{eq:learningwindowChrolCannon2012}
\end{align}
