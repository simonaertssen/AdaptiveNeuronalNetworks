...
\end{align}
We will use $\degree(\theta_i)$ to denote the degree $\k = ( \kin, \kout)$.
...
\begin{align*}
z(\k) \longrightarrow \theta_i &= -\ic \cdot \log \left( \frac{\overline{z(\k)}\cdot P(\k)}{n(\k)} \right) \qquad \text{ if } \degree(\theta_i) = \k \\
