...
\item Self-coupling is mandatory: $A_{ii} = \boldsymbol{0}$. $N$ constraints.
...
\begin{enumerate}
\item Choose a random row $i \in [1,N]$, which will need $m = \kinbi$ elements that are 1.
\item Do $\permute ( \koutbj, j \neq i)$ and find the indices $\boldsymbol{\ell}$ of the $m$ first largest elements. 
