...
\begin{align}
\dot{\theta} = (1-\cos \theta)+(1+\cos \theta) \cdot \iota \label{eq:thetaneuron}
\end{align}
with $\iota$ a bifurcation parameter on the supplied current. We can visualise the dynamics on the unit circle, like in Figure \ref{fig:thetaneuronbifurcationtikz}. The neuron produces a spike when $\theta$ surpasses $\pi$. As $\iota$ increases, we see the coalescence of a saddle and node and the neuron starts to fire periodically, we can recognise the features of the class 1 model in Figure. This makes \eqref{eq:thetaneuron} the normal form of the \textit{saddle-node-on-invariant-circle} ($\SNIC$) bifurcation \cite{Luke2013}.\\

We can easily extend the model to networks of neurons:
\begin{align}
\dot{\theta}_{i} &=\left(1-\cos \theta_{i}\right)+\left(1+\cos \theta_{i}\right) \cdot \left[\eta_{i} + \kappa I_{i}(t)\right] \label{eq:thetaneuronnetwork} \\
I_{i}(t) &=\frac{1}{\k} \sum_{j=1}^{N} A_{i j} \cdot \mathcal{P}_{n}(\theta_{j}) \label{eq:thetaneuronnetworkcurrent}
\end{align}
...
\end{tikzpicture}
\endminipage
\caption{SNIC bifurcation of the theta neuron model. For $\iota < 0$, the neuron is in a rest state but excitable. For $\iota > 0$, the neuron spikes regularly. The bifurcation occurs at $\iota = 0$. A spike occurs when $\theta = \pi$.}
