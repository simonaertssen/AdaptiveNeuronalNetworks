...
Gaining insight into \eqref{eq:thetaneuron} is hard, due to the difficulty of finding an analytical solution. However, it has been noted that there exists a simple transformation $V \equiv \tan \frac{\theta}{2}$ which yields $\dot{V} = V^2 + I$, see \ref{app:TransformationToQIF}. This model is also called the \textsl{Quadratic Integrate and Fire model} (\QIF). $V$ and $\theta$ are both continuous on $\T$, so insights from a solution for $V$ can be transformed directly. The solution is (see \ref{app:ThetaModelSolutions}):
