...
\end{align}
The \PRC is thus the expected delay of the the phase period in function of the timing of the delay. This process has been visualised in Figure \ref{fig:ThetaNeuronPRC}, after \cite{Perez2020}. For infinitesimally small perturbations to the phase, we can find the \PRC as the \textsl{adjoint} of the solution \cite{Gutkin2014}:
\begin{align}
...
A formulation for $T_{\phi}$ is obtained by integrating after the bifurcation. The \PRC then becomes:
\begin{align}
...
\end{align}
When plotting \eqref{eq:PRCepsilon} for different values of $\varepsilon$ in Figure \ref{fig:ThetaNeuronPRC}, we can see that larger bifurcations yield larger delays on the phase. For infinitesimally small bifurcations the response in \eqref{eq:PRC2} is symmetric about the middle of the period. Here we see that for larger bifurcations the \PRC is skewed to the left, which means that the point where perturbation of the neuron is greatest is briefly after a spike. This tendency continues to skew left, and the \PRC converges to a value of $T$ at $\phi = 0$, as a delay with $T$ is really the largest delay we can achieve with one bifurcation.

\begin{figure}[H]
\centering
\includegraphics[width = \textwidth]{../Figures/ThetaNeuronPRC.pdf}
\caption{Response of the Theta model to bifurcations on the phase. Left: a bifurcation $\varepsilon  < 0 $ at time $\phi$ perturbs $\theta(t)$ (in blue) which results in a delayed spike (trajectory in red). Right: the \PRC, given by \eqref{eq:PRCepsilon}, plotted for in orange for $\varepsilon$ ranging from 0.1 to 1 - a weak bifurcation is more transluscent. A solution for $\theta$ (in blue) shows when the model is the most susceptible to bifurcations over the course of one period.}
