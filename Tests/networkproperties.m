...
for i = 1:nsamples
    ...
    [L, ~, CClosed, ~, COpen, ~] = graphproperties(initarray(double(adjacencymatrix(fdpars.degrees_in))));
    Ls(i) = L;
    CCs(i) = CClosed;
    COs(i) = COpen;
    i
...
for i = 1:nsamples
    ...
    [L, ~, CClosed, ~, COpen, ~] = graphproperties(initarray(double(adjacencymatrix(randompars.degrees_in))));
    Ls(i) = L;
    CCs(i) = CClosed;
    COs(i) = COpen;
    i
...
for i = 1:nsamples
    ...
    [L, ~, CClosed, ~, COpen, ~] = graphproperties(initarray(double(adjacencymatrix(sfpars.degrees_in))));
    Ls(i) = L;
    CCs(i) = CClosed;
    COs(i) = COpen;
    i
